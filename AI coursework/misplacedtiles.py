...
def misplaced_tiles_heuristic(current_state:list, goal_state:list)->int:
    """Heuristic function to find the distance to the goal state using the misplaced tiles technique

    Args
    current_state - The current_state of the puzzle
    goal_state - The goal state of the puzzle

    Returns an integer value for the number of misplaced tiles"""
    misplaced_tiles = 0
    for row in range(0, 3):
        for tile in range(0,3):
            if current_state[row][tile] != goal_state[row][tile]:
                misplaced_tiles +=1
...
def a_star(start_node: Node, end_node: Node):
    """Performs A* path finding between two nodes using misplaced tiles as the heuristic function.
